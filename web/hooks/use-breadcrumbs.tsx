...
function buildFromPath(pathname: string, overrides: BreadcrumbOverrides): BreadcrumbItem[] {
...
export function BreadcrumbsProvider({ children }: { children: React.ReactNode }): React.ReactElement {
  ...
  const setOverrides = useCallback((overrides: BreadcrumbOverrides) => {
  ...
  const push = useCallback((item: BreadcrumbItem) => {
    setManualTrail((prev) => (prev ? [...prev, item] : [
      ...buildFromPath(pathname ?? "/", overridesRef.current),
      item,
    ]));
  }, [pathname]);
