...
export function monthRangeIST(year: number, month0: number): { start: Date; end: Date } {
  const start = new Date(new Date(Date.UTC(year, month0, 1)).toLocaleString("en-US", { timeZone: IST_TIMEZONE }))
  const end = new Date(new Date(Date.UTC(year, month0 + 1, 0)).toLocaleString("en-US", { timeZone: IST_TIMEZONE }))
  return { start, end }
}
